...
class bdi2 extends Controller
{
    ...
    if ($sum>=1&&$sum<10) {
        $ans = '<p>امتیاز: '.$sum.'</p><p>میزان افسردگی طبیعی</p>';
    } else if ($sum>=11&&$sum<16) {
        $ans = '<p>امتیاز: '.$sum.'</p><p>شما کمی افسرده هستید</p>';
    } else if ($sum>=17&&$sum<20) {
        $ans = '<p>امتیاز: '.$sum.'</p><p>نیازمند مشورت با روانپزشک</p>';
    } else if ($sum>=21&&$sum<30) {
        $ans = '<p>امتیاز: '.$sum.'</p><p>به نسبت افسرده</p>';
    }else if ($sum>=31&&$sum<40) {
        $ans = '<p>امتیاز: '.$sum.'</p><p>افسردگی شدید</p>';
    } else if ($sum>=41) {
        $ans = '<p>امتیاز: '.$sum.'</p><p>افسردگی بیش از حد</p>';
    }

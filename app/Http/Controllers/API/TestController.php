...
class TestController extends Controller
{
    ...
    public function show_tests(Request $request)
    {
        if (isset($request->test_id) && isset($request->question_number)) {
            $number=Test::find($request->test_id)->question_number;
        ...
        if (isset($request->test_id) && !isset($request->question_number)){
            ...
            return response()->json(['success' => $t], $this->successStatus);
        }
        ...
        foreach ($tests as $test){
            ...
            if(UserTestAnswer::where('user_id',Auth::user()->id)->where('test_id',$test->id)->count()==0) {
                $t['last_answered_question_number'] = null;
                    ...
                    $test_question_idd = UserTestAnswer::where('user_id', Auth::user()->id)->where('test_id', $test->id)->orderBy('test_question_id', 'DESC')->limit(1)->get();
                    foreach ($test_question_idd as $ttt){
                        $test_question_id=$ttt->test_question_id;
                    }
                    $t['last_answered_question_number'] = TestQuestion::find($test_question_id)->question_number;
...
//        $body = $response->getBody();
        ...
        $text='
<img src="http://shavernoapp.ir/images/shsh.png" width="200px">
<p>تست شما ثبت شد و پاسخ آن برایتان ارسال می گردد. به زودی قابلیت تصحیح تست به صورت لحظه ای در اپلیکیشن شاورنو اضافه خواهد شد</p>';

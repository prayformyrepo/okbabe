...
//        $conversation=Conversation::select('id','text','status','updated_at')->find($request->conversation_id);
        ...
        $messages = $conversation->messagesByLastId($last_message_id)->paginate(10000);
        $mess = array();
        foreach ($messages as $m) {
            $me['id'] = $m->id;
            $me['text'] = $m->text;
            $me['file_id'] = $m->file_id;
            $me['file_name'] = File::find($me['file_id'])->orginal_name;
            $me['file_path'] = File::find($me['file_id'])->file_path;
            $me['file_type'] = File::find($me['file_id'])->file_type;
